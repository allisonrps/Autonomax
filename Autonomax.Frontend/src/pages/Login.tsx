...
export function Login() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gray-950 font-sans">
      
      {/* LADO ESQUERDO: VITRINE (Padrão Original) */}
      <div className="hidden md:flex md:w-5/12 lg:w-1/2 bg-emerald-950 relative overflow-hidden flex-col justify-between p-12 lg:p-16">
        ...
        <div className="z-10">
           ...
           <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-y-8 gap-x-12">
             {[ {i: Briefcase, t: 'Multi-Negócios'}, {i: TrendingUp, t: 'Fluxo de Caixa'}, {i: Users, t: 'Gestão de Clientes'}, {i: Truck, t: 'Parceiros & Custos'}, {i: BarChart3, t: 'Estatísticas'}, {i: Package, t: 'Top Performance'} ].map((f, idx) => (
                <div key={idx} className="flex gap-4"><div className="p-3 bg-white/5 rounded-2xl text-emerald-400 h-fit"><f.i size={22}/></div><div><h4 className="text-white font-bold text-sm">{f.t}</h4></div></div>
             ))}
           </div>
        </div>
      ...
      {/* LADO DIREITO: LOGIN (Padrão Dark) */}
      <div className="flex-1 flex items-center justify-center p-8 bg-gray-950">
        <div className="max-w-md w-full bg-gray-900 p-8 rounded-xl border border-gray-800 shadow-2xl space-y-8">
          <div className="flex justify-center">
            <img src={logoImg} alt="Autonomax" className="h-16 w-auto object-contain" />
          </div>

          <div className="space-y-1 text-center">
            <h3 className="text-xl font-black text-white uppercase tracking-widest">Acesso ao Sistema</h3>
            <p className="text-xs text-gray-500 font-medium">Insira suas credenciais.</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input type="email" required className="w-full pl-10 pr-4 py-3 bg-gray-950 border border-gray-800 rounded-md focus:border-emerald-600 outline-none font-bold text-sm text-white transition-all placeholder-gray-700" placeholder="seu@email.com" value={email} onChange={e => setEmail(e.target.value)} />
              </div>
            </div>

            <div className="space-y-1">
              <div className="flex justify-between items-center px-1">
                <label className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Senha</label>
                <button type="button" className="text-[9px] font-black text-emerald-500 uppercase hover:text-emerald-400">Recuperar</button>
              </div>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" size={16} />
                <input type="password" required className="w-full pl-10 pr-4 py-3 bg-gray-950 border border-gray-800 rounded-md focus:border-emerald-600 outline-none font-bold text-sm text-white transition-all placeholder-gray-700" placeholder="••••••••" value={senha} onChange={e => setSenha(e.target.value)} />
              </div>
            </div>
          ...
          <div className="pt-4 border-t border-gray-800 text-center">
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">
              Não possui conta? <button type="button" onClick={() => window.location.href = '/register'} className="text-emerald-500 hover:text-emerald-400 font-black ml-1 uppercase">Cadastre-se</button>
            </p>
          </div>
        </div>
      </div>
