...
export function Relatorios() {
  ...
  // ESTADOS PARA OS MENUS RETRÁTEIS INDEPENDENTES
      ...
      <div className="max-w-6xl mx-auto space-y-8 pb-16 pt-8 px-4 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-1"><ArrowUpRight size={12} className="text-emerald-500"/> Total Receitas</p><p className="text-2xl font-black text-emerald-600">R$ {totalEntradas.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
            <div className="bg-emerald-50 p-3 rounded-2xl text-emerald-600"><TrendingUp size={20} /></div>
          </div>
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2 mb-1"><ArrowDownLeft size={12} className="text-red-500"/> Total Despesas</p><p className="text-2xl font-black text-red-500">R$ {totalSaidas.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
          ...
          <div className="bg-emerald-950 p-6 rounded-3xl shadow-xl flex items-center justify-between text-white">
            <div><p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">Resultado Líquido</p><p className="text-2xl font-black text-emerald-50">R$ {faturamentoLiquido.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
        ...
        <div className="bg-white rounded-3xl border border-gray-200 shadow-xl overflow-hidden">
          <button onClick={() => setGraficoAberto(!graficoAberto)} className="w-full bg-emerald-50/50 px-8 py-5 border-b border-gray-100 flex items-center justify-between hover:bg-emerald-50 transition-colors border-none outline-none cursor-pointer">
            <div className="flex items-center gap-3">
              <PieChart size={20} className="text-emerald-600" />
              <h3 className="text-xs font-black text-emerald-900 uppercase tracking-[0.2em]">Fluxo de Caixa Mensal</h3>
            </div>
              ...
              <div className="h-[350px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={dadosGrafico} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Tooltip cursor={{fill: '#f0fdf4'}} contentStyle={{borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)'}} />
                    ...
                    <Bar dataKey="lucro" name="Lucro" fill="#064e3b" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
        ...
        {/* 3. RANKING DE CLIENTES RETRÁTIL */}
        <div className="bg-white rounded-3xl border border-gray-200 shadow-xl overflow-hidden">
