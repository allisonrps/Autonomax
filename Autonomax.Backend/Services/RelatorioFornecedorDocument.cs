...
public class RelatorioFornecedorDocument : IDocument
{
    private readonly RelatorioFornecedorDto _dados;
    ...
    private readonly string _dangerColor = "#dc2626";    // Red 600

    public RelatorioFornecedorDocument(RelatorioFornecedorDto dados) => _dados = dados;
    ...
    public void Compose(IDocumentContainer container)
    {
        container.Page(page =>
        {
            ...
            page.Footer().AlignCenter().Column(col => 
            {
                ...
                {
                    row.RelativeItem().Text("Autonomax - Gestão de Fluxo Inteligente").FontSize(8).FontColor(Colors.Grey.Medium);
    ...
    void ComposeHeader(IContainer container)
    {
        container.Column(col =>
        {
            ...
            // Subtítulos
            col.Item().AlignCenter().Text("Histórico de Gastos do Parceiro").FontSize(14).Bold().FontColor(_darkColor);
    ...
    void ComposeContent(IContainer container)
    {
        ...
        {
            // --- BOX DE INFORMAÇÕES DO FORNECEDOR ---
            column.Item().Row(row =>
            {
                row.RelativeItem().Column(col =>
                {
                    col.Item().Text("INFORMAÇÕES DO PARCEIRO").FontSize(8).Bold().FontColor(_primaryColor);
                    col.Item().PaddingTop(2).Text(_dados.NomeFornecedor).FontSize(16).ExtraBold();
                    col.Item().Text($"Contato: {_dados.Telefone}").FontSize(10);
                    if (!string.IsNullOrWhiteSpace(_dados.Categoria))
                    {
                        col.Item().Text($"Categoria: {_dados.Categoria}").FontSize(9).FontColor(Colors.Grey.Darken1);
                    }
                });

                row.RelativeItem().AlignRight().Column(col =>
                {
                    col.Item().Text("RESUMO OPERACIONAL").FontSize(8).Bold().FontColor(_primaryColor);
                    col.Item().PaddingTop(2).Text($"{_dados.Transacoes.Count} lançamentos registrados").FontSize(10);
                    col.Item().Text("Período: Histórico Geral").FontSize(9).Italic();
                });
            });
            ...
            column.Item().Table(table =>
            {
                ...
                foreach (var item in _dados.Transacoes)
                {
                    ...
                    table.Cell().Element(RowStyle).AlignRight().Text($"{item.Valor:N2}").FontColor(_dangerColor);
            ...
            column.Item().PaddingTop(15).Row(row => 
            {
                ...
                row.ConstantItem(200).Background(_softGrey).Padding(10).Column(col => 
                {
                    col.Item().Row(r => 
                    {
                    ...
                    {
                        r.RelativeItem().Text("Total Pago / Gasto").FontSize(10);
                        r.RelativeItem().AlignRight().Text($"R$ {_dados.TotalGasto:N2}")
                            .FontSize(12).ExtraBold().FontColor(_dangerColor);
                    });
                });
            });
        });
